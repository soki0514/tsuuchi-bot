...
def _get_sol_price_usd():
    ...
    try:
        ...
        if r.status_code == 200:
            price = float(r.json().get("data", {}).get("SOL", {}).get("price", 0))
            if price > 0:
...
def _get_v2_pair_liquidity_usd(pair_addr, token0, token1, chain):
    try:
        ...
        if t1 in STABLE_ADDRS: return reserve1 / (10 ** STABLE_DECIMALS[t1])
        if t0 in STABLE_ADDRS: return reserve0 / (10 ** STABLE_DECIMALS[t0])
...
def _get_v3_pool_liquidity_usd(pool_addr, token0, token1, chain):
    ...
    try:
        t0, t1 = token0.lower(), token1.lower()
        if t1 in STABLE_ADDRS: return _balance_of(t1) / (10 ** STABLE_DECIMALS[t1])
        if t0 in STABLE_ADDRS: return _balance_of(t0) / (10 ** STABLE_DECIMALS[t0])
        if t1 in NATIVE_ADDRS:
            p = _get_native_price_usd(chain)
...
def _wait_for_liquidity_mint(pair_addr, token_address, chain, from_block, token0, token1, is_v2):
    ...
    while time.time() < deadline:
        ...
        liq = (_get_v2_pair_liquidity_usd if is_v2 else _get_v3_pool_liquidity_usd)(
            pair_addr, token0, token1, chain)
        ...
        holder_result = [None]
        dex_result    = [None]
        def _fetch_holder(): holder_result[0] = get_evm_holder_stats(token_address, chain, from_block)
        def _fetch_dex():    dex_result[0]    = analyze_dexscreener(token_address)
        t_h = threading.Thread(target=_fetch_holder, daemon=True)
        t_d = threading.Thread(target=_fetch_dex,    daemon=True)
        t_h.start(); t_d.start(); t_h.join(); t_d.join()
        ...
        dex_extra   = (f"📈 価格変動: {dex['price_change_5m']:+.1f}%/5分\n"
        ...
        msg = (f"{chain['emoji']} <b>[{chain['name']}] 新規トークン検知！</b>\n\n"
...
def _process_evm_token(token_address, chain, from_block,
    ...
    try:
        if pair_addr and token0 and token1:
            ...
            print(f"[{chain['name']}] オンチェーン流動性: ${'%,.0f' % liq if liq else '0'} ({token_address[:12]})")
            if liq is None or liq < LIQUIDITY_MIN:
...
def check_evm_chain(chain):
    try:
        ...
        for log in logs:
            ...
            threading.Thread(target=_process_evm_token,
                             args=(token_address, chain, latest_int), daemon=True).start()
    except Exception as e:
    ...
    try:
        ...
        for log in logs:
            ...
            threading.Thread(
                ...
                kwargs={"pair_addr": pair_addr, "token0": token0, "token1": token1, "is_v2": is_v2},
                daemon=True,
            ).start()
    except Exception as e:
        print(f"[{chain['name']}] チェックエラー: {e}")
...
def get_new_pumpfun_transactions():
    ...
    while True:
        ...
        if last_signature: opts["until"]  = last_signature
...
def parse_new_token(signature):
    ...
    pre_mints     = {b.get("mint") for b in result.get("meta", {}).get("preTokenBalances", [])}
...
def format_holder_output(holder_data):
    ...
    text = (f"👛 <b>保有者分析</b> (実際の保有量)\n"
            f"上位10保有者合計: {holder_data['top10_ratio']:.0f}%\n{top10_lines}\n")
    r = holder_data["top10_ratio"]
    judge = "🚨 保有集中度高め" if r >= 50 else ("✅ 分散した保有" if r < 30 else "🟡 やや集中気味")
    return text, judge
...
def get_evm_holder_stats(token_address, chain, from_block):
    try:
        ...
        for log in logs:
            ...
            if amount == 0:
                continue
            if from_addr != ZERO_ADDR: balances[from_addr] = balances.get(from_addr, 0) - amount
            if to_addr   != ZERO_ADDR: balances[to_addr]   = balances.get(to_addr,   0) + amount
        ...
        top10_detail   = [
            f"  {'ABCDEFGHIJ'[i]}. {addr[:6]}...{addr[-4:]}: {bal/total*100:.1f}%"
            for i, (addr, bal) in enumerate(top10)
        ]
        return {"top10_ratio": top10_total / total * 100, "top10_detail": top10_detail}
    except Exception as e:
...
# ══════════════════════════════════════════════════════════════════════════════
# Solana トークン処理スレッド（pump.fun / Solana全般 共通）
# ══════════════════════════════════════════════════════════════════════════════
            ...
            if effective_pump:
                ...
                if pf:
                    ...
                    if liq >= LIQUIDITY_MIN:
                        ...
                        msg = (f"🚀 <b>[Solana/{platform}] 新規トークン検知！</b>\n\n"
                               f"時刻: {datetime.now().strftime('%H:%M:%S')}\n{token_name}"
                               f"📋 Mintアドレス（タップでコピー）\n<code>{mint}</code>\n\n"
                               f"{dex_text}{holder_text}\n{holder_judge}\n\n"
...
def _handle_pumpfun_sig(sig):
    ...
    threading.Thread(target=_wrapped_process_solana, args=(mint,), daemon=True).start()
...
def check_pumpfun_onchain():
    ...
    txns = [tx for tx in txns if not tx.get("blockTime") or (now - tx["blockTime"]) <= 300]
    if len(txns) < before_filter:
        print(f"[Pump.fun] 古いTX除外: {before_filter - len(txns)}件スキップ（残り{len(txns)}件）")
    sigs = [tx.get("signature", "") for tx in txns if tx.get("signature") and not tx.get("err")]
    if not sigs:
        return
...
def get_new_metadata_transactions():
    ...
    while True:
        opts = {"limit": 50, "commitment": "confirmed"}
        if all_solana_last_signature: opts["until"]  = all_solana_last_signature
        if before:                    opts["before"] = before
...
def _handle_metadata_sig(sig):
    ...
    threading.Thread(target=_wrapped_process_solana,
                     args=(mint, "Solana全般", False), daemon=True).start()
...
def parse_raydium_new_pool(signature):
    ...
    for b in post_balances:
        mint = b.get("mint", "")
        if not mint or mint in pre_mints or mint in SOLANA_BASE_TOKENS:
            continue
        new_mint = mint
        break
    if not new_mint:
...
def _process_raydium_token(mint, liq_usd):
    try:
        if liq_usd >= LIQUIDITY_MIN:
            ...
            holder_text, holder_judge = format_holder_output(holder_data)
            msg = (f"⚡ <b>[Solana/Raydium] 新規トークン検知！</b>\n\n"
                   f"時刻: {datetime.now().strftime('%H:%M:%S')}\n"
                   f"📋 Mintアドレス（タップでコピー）\n<code>{mint}</code>\n\n"
                   f"{dex_text}{holder_text}\n{holder_judge}\n\n"
                   f"📊 https://dexscreener.com/solana/{mint}\n"
...
def _handle_raydium_tx(sig):
    ...
    threading.Thread(target=_wrapped_process_raydium, args=(mint, liq_usd), daemon=True).start()
...
def get_bitget_contract_addresses(coin):
    try:
        ...
        if not coins:
            return {}
        return {
            chain.get("chain", ""): chain.get("contractAddress", "")
            for chain in coins[0].get("chains", [])
            if chain.get("chain") and chain.get("contractAddress")
        }
    except Exception as e:
...
def check_cex_listings():
    ...
    for symbol in current - known_cex_symbols:
        if symbol.endswith("USDT"):
            ...
            addr_text = ""
            if addresses:
                lines     = [f"  <b>{cn}:</b> <code>{addr}</code>" for cn, addr in list(addresses.items())[:5]]
                addr_text = "📋 コントラクトアドレス:\n" + "\n".join(lines) + "\n\n"
            msg = (f"🏦 <b>[CEX] Bitget新規上場！</b>\n\n"
                   f"トークン: <b>${base}</b>\n"
                   f"時刻: {datetime.now().strftime('%H:%M:%S')}\n\n"
                   f"{addr_text}✅ Bitget審査済み（比較的安全）\n"
...
def main():
    ...
    threading.Thread(target=pumpfun_monitor_loop,    daemon=True).start()
    print("[Pump.fun] バックグラウンドスレッド起動完了")
    threading.Thread(target=solana_all_monitor_loop, daemon=True).start()
    print("[Solana全般] バックグラウンドスレッド起動完了")
    threading.Thread(target=raydium_monitor_loop,    daemon=True).start()
    ...
    while True:
        ...
        if loop % 360 == 0:
            ...
            print(f"[{datetime.now().strftime('%H:%M')}] 稼働中 "
