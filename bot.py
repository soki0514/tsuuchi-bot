...
def _build_wallet_stats(wallets):
    ...
    top10_detail = []
    top10_total  = 0
    for i, (addr, cnt) in enumerate(top10):
        r     = cnt / total * 100
        short = addr[:6] + "..." + addr[-4:]
        top10_detail.append(f"  {'ABCDEFGHIJ'[i]}. {short}: {r:.1f}% ({cnt}件)")
        top10_total += cnt

    top10_ratio   = top10_total / total * 100 if total > 0 else 0
    others_cnt    = total - top10_total
    others_unique = max(0, unique - len(top10))
    if others_cnt > 0:
        top10_detail.append(
...
def _process_evm_token(token_address, chain):
    try:
        ...
        holder_data = get_evm_holder_stats(token_address, chain, first_block)
        top10_ratio = holder_data["top10_ratio"] if holder_data else 0
        print(f"[{chain['name']}] 保有量トップ10比率: {top10_ratio:.1f}%")
        if top10_ratio >= 60:
            print(f"[{chain['name']}] ❌ 保有集中度高すぎ（{top10_ratio:.1f}% >= 60%）→ 通知スキップ")
            return

        print(f"[{chain['name']}] ✅ フィルター合格！通知送信中...")
        dex = analyze_dexscreener(token_address)
        if dex and dex['liquidity'] <= 500:
            print(f"[{chain['name']}] 流動性不足 (${dex['liquidity']:,.0f} <= $500) → スキップ")
            return
...
def check_evm_chain(chain):
    try:
        ...
        for log in logs:
            ...
            t = threading.Thread(target=_process_evm_token, args=(token_address, chain), daemon=True)
            t.start()
...
def check_evm_all_chain(chain):
    try:
        ...
        for log in logs:
            ...
            t = threading.Thread(target=_process_evm_all_token, args=(new_token, chain), daemon=True)
            t.start()
...
def is_top10_concentrated(wallet_data, label=""):
    if not wallet_data:
        return False
    top10_ratio = wallet_data.get("top10_ratio", 0)
    print(f"[{label}] トップ10比率: {top10_ratio:.1f}%")
    if top10_ratio >= 60:
        print(f"[{label}] ❌ トップ10集中度高すぎ（{top10_ratio:.1f}% >= 60%）→ 通知スキップ")
        return True
    return False
...
def is_holder_ratio_suspicious(mint, unique_traders, label=""):
    """
    保有者分布が不自然かチェック。
    条件: 保有者数<=1000 かつ 保有者数/取引アドレス数>=5 → True（通知スキップ）
    ...
    if holder_count <= 1000 and ratio >= 5:
        print(f"[{label}] ❌ 保有者比率異常（{ratio:.1f}倍 >= 5倍）→ 通知スキップ")
        return True
    return False
...
def _process_solana_token(mint):
    try:
        ...
        if early_unique >= 30:
            ...
            if dex and dex['liquidity'] <= 500:
                print(f"[Pump.fun早期] 流動性不足 (${dex['liquidity']:,.0f} <= $500) → スキップ")
                return
            dex_text = (
                f"💧 流動性: ${dex['liquidity']:,.0f}\n"
                f"📈 価格変動: {dex['price_change_5m']:+.1f}%/5分\n"
        ...
        holder_data = get_solana_holder_stats(mint)
        top10_ratio = holder_data["top10_ratio"] if holder_data else 0
        print(f"[Pump.fun確定] 保有量トップ10比率: {top10_ratio:.1f}%")
        if top10_ratio >= 60:
            print(f"[Pump.fun確定] ❌ 保有集中度高すぎ（{top10_ratio:.1f}% >= 60%）→ 通知スキップ")
            return
        ...
        dex = analyze_dexscreener(mint)
        if dex and dex['liquidity'] <= 500:
            print(f"[Pump.fun確定] 流動性不足 (${dex['liquidity']:,.0f} <= $500) → スキップ")
            return
        holder_text, holder_judge = format_holder_output(holder_data)
        dex_text = (
            f"💧 流動性: ${dex['liquidity']:,.0f}\n"
...
def _process_solana_any_token(mint):
    try:
        ...
        if early_unique >= 50:
            ...
            dex = analyze_dexscreener(mint)
            if dex and dex['liquidity'] <= 500:
                print(f"[Solana全般早期] 流動性不足 (${dex['liquidity']:,.0f} <= $500) → スキップ")
                return
        ...
        holder_data = get_solana_holder_stats(mint)
        top10_ratio = holder_data["top10_ratio"] if holder_data else 0
        print(f"[Solana全般確定] 保有量トップ10比率: {top10_ratio:.1f}%")
        if top10_ratio >= 60:
            print(f"[Solana全般確定] ❌ 保有集中度高すぎ（{top10_ratio:.1f}% >= 60%）→ 通知スキップ")
            return
        ...
        dex = analyze_dexscreener(mint)
        if dex and dex['liquidity'] <= 500:
            print(f"[Solana全般確定] 流動性不足 (${dex['liquidity']:,.0f} <= $500) → スキップ")
            return
        platform = _get_platform_name(dex)
        dex_text = _build_dex_text(dex)
        holder_text, holder_judge = format_holder_output(holder_data)
...
def check_all_solana_onchain():
    ...
    for tx_info in txns:
        ...
        t = threading.Thread(target=_process_solana_any_token, args=(mint,), daemon=True)
        t.start()
