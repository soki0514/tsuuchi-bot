...
def _process_evm_token(token_address, chain, from_block,
    ...
    pair_addr あり（check_evm_all_chain から）:
        ...
        万が一 reserve=0 なら 1秒 × 3回 リトライ後DexScreenerフォールバック。
        → 平均通知まで 8〜15秒。

    pair_addr なし（check_evm_chain / FourMeme・Clanker / フォールバック）:
    ...
    try:
        ...
        if pair_addr and token0 and token1:
            ...
            # liq=0 または LIQUIDITY_MIN未満 → DexScreenerフォールバック
            # 流動性が後から追加されるトークンを拾い直す（従来はここでスキップ → 永遠に通知なし）
            if liq is None or liq < LIQUIDITY_MIN:
                print(f"[{chain['name']}] オンチェーン流動性不足(${liq or 0:,.0f}) → DexScreenerへフォールバック: {token_address[:12]}")
                # pair_addr=None扱いでDexScreenerポーリングへ続行（下のブロックへ）
                pair_addr = None
                token0    = None
                token1    = None
                # fall through to DexScreener polling below
...
# ══════════════════════════════════════════════════════════════════════════════
# Solana トークン処理スレッド（閾値ベース: 流動性$10k+ かつ トップ10≤60%）
# pump.fun / Solana全般 共通
# ══════════════════════════════════════════════════════════════════════════════

def _process_solana_token(mint, label="Pump.fun", pump_link=True):
    """
    新規Solanaトークンを監視し、流動性$10k+ かつ トップ10保有率≤60% で即通知。
