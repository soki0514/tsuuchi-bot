...
def get_holder_count(mint_address):
    """
    Solanaトークンの保有者数を取得。Helius DAS API getTokenAccounts を使用。
    """
    if not HELIUS_KEY:
        return None
...
def _process_solana_token(mint):
    """
    新規Solanaトークンの2段階通知。
    STEP A (60秒後): ユニーク30人以上 → 🟣 早期通知
    STEP B (180秒後): ユニーク20人以上 → 🚀 確定通知
    """
    try:
