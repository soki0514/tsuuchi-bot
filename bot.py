...
def _process_evm_token(token_address, chain, from_block,
    ...
    try:
        ...
        # ── フォールバック: DexScreenerポーリング（FourMeme/Clanker用）─────────
        deadline = time.time() + MONITOR_TIMEOUT_SEC
...
# ══════════════════════════════════════════════════════════════════════════════
# EVM 全般監視 (PancakeSwap V2/V3 / Uniswap V3 PoolCreated)
# ══════════════════════════════════════════════════════════════════════════════
...
def main():
    ...
    send_telegram(
        ...
        f"⚡ 新戦略: Mintイベント直接検知（DexScreener遅延排除）\n"
        f"💧 初期流動性 ${LIQUIDITY_MIN:,}+ AND\n"
        f"👛 トップ10保有率 ≤{TOP10_MAX_PCT:.0f}%\n"
        f"🔄 EVM検知5秒毎 / BNB・Base通知まで平均5〜10秒\n\n"
        "🔍 Solana監視対象：\n"
