...
# {address_lower: {"chain": "sol"/"evm", "registered_at": float,
#                  "source": str, "notified": bool}}
_ca_watch      = {}
...
# 全スレッド合計で同時にgetTransactionを呼べるのは6つまで。
# _SOLANA_RPS_LIMIT=6 と揃えることで同時RPC数がRPS制限を超えないよう制御する。
...
# ── Solana RPCグローバルレート制限（6 RPS）────────────────────────────────────
# 全スレッド合計で1秒間に呼べるSolana RPC数の上限。
# 6本のRPC（Helius/Publicnode×2/公式/drpc/Omnia）にラウンドロビンするため
# 1本あたり4 RPS × 6本 = 24 RPS を上限とする。
...
def _has_token_icon(key: str, chain: str, dex: dict = None) -> bool:
    ...
    【Solana】:
      ...
      2. pump.fun API の image_uri     → 5〜10秒
...
def _extract_and_register_ca(text: str, source: str):
    """テキストからEVM/SolanaアドレスをすべてCA監視に登録する。"""
    # EVM（0x...）: キーワード不要（形式が明確）
            ...
            for i in range(0, len(keys), BATCH_SIZE):
                ...
                for key in batch:
                    ...
                    if liq < LIQUIDITY_MIN:
                        ...
                        # まだ一度もpump.fun APIチェックしていないトークンのみ起動。
                        if (info.get("chain") == "sol"
                                and not info.get("pumpfun_spawned")):
                            with _pending_lock:
                                if (key in _pending_tokens
                                        and not _pending_tokens[key].get("pumpfun_spawned")):
                                    _pending_tokens[key]["pumpfun_spawned"] = True
...
def ca_watch_loop():
    ...
    流動性が LIQUIDITY_MIN 以上になった瞬間に通知。
    24時間経過で自動削除。
    """
    while True:
        try:
...
def telegram_sns_monitor_loop():
    ...
    ③ 監視対象グループはTELEGRAM_MONITOR_CHATSで設定
       （空の場合はプライベートチャットの /watch のみ有効）
    """
    ...
    while True:
        try:
            ...
            for upd in updates:
                ...
                chat_type = msg.get("chat", {}).get("type", "")  # private/group/supergroup/channel
...
def _startup_scan_solana(now, min_age, max_age):
    ...
    Raydium AMM V4 / CPMM を対象に過去20分〜8時間のプール作成TXを解析。
